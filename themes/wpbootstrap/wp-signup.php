...
if ( !is_main_site() ) {
	wp_redirect( network_site_url( '<?php echo $permalink; ?>/wp-signup.php' ) );
	die();
}
...
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	if ( !is_user_logged_in() ) {
		...
		echo '<p>(<strong>' . sprintf( __('Your temporary address will be %s.'), $site ) . '</strong>) ' . __( '<br />Must be at least 4 characters, letters and numbers only. It cannot be changed, so choose carefully!<br/>This address will be used to preview to site while it is being built.' ) . '</p>';
	}
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array() ) {
	?>
	<div id="topside" align="left"><h2><div class="selected">Choose Your Plan > <span class="selectedon">Pick Your Username</span> >  Create Your Site </div></h2></div>
					...
					<h4 class="yoursite"><?php printf( __( 'This will be used to log into your site.' ), get_current_site()->site_name ) ?></h4>
						<form id="setupform" class="span9" method="post" action="<?php echo $permalink; ?><?php echo $permalink; ?>/wp-signup.php" novalidate="novalidate">
							...
							<br />
								...
								<label class="checkbox" for="signupblog"><?php _e('*Accept <a href="#">Terms of Service</a>') ?></label>
								<br />
							<?php } ?>
							</p>
...
function signup_blog($user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '') {
	...
	?>
					...
					<div id="topside" align="left">
						<h2>
							<div class="selected">Choose Your Plan > Pick Your Username >  
								<span class="selectedon">
									Create Your Site 
								</span>
							</div>
						</h2>
		...
		<div class="container">
			<div class="row">
						...
						<form id="setupform" method="post" class="span9" action="<?php echo $permalink; ?>/wp-signup.php">
							...
							<?php show_blog_form($blogname, $blog_title, $errors); ?>
							<br/>
							<p class="submit"><input type="submit" name="submit" class="btn btn-primary btn-block" value="<?php esc_attr_e('Create') ?>" /></p>
...
function confirm_blog_signup( $domain, $path, $blog_title, $user_name = '', $user_email = '', $meta = array() ) {
	...
	<div id="topside" align="left"><h2><div class="selected sele"><span class="selectedon">We're Here to Help - <a href="#">Contact Us</a></span></div></h2></div>
		...
		<div class="container">
			<div class="row">
				<div class="span12">
					...
					<p><?php _e( 'But, before you can access your site, <strong>you must activate it</strong>.' ) ?></p>
					<p><?php printf( __( 'Check your inbox at <strong>%s</strong> and click the link given.' ),  $user_email) ?></p>
					...
					<p>
						...
						<ul id="noemail-tips">
							...
							<li><?php printf( __( 'Have you entered your email correctly? You have entered <strong>%s</strong>, if it&#8217;s incorrect, you will not receive your email.' ), $user_email ) ?></li>
						</ul>
					</p>
				</div>
			</div>
